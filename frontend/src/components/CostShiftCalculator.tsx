 ...
 * and CO2" requirement. Region and the load profile come from the
 ...
 * window), deliberately decoupled from PriceChart's -
 * see ShiftCalculator's doc comment for the rationale. Always runs
  ...
  return (
    <div className="space-y-8">
      <p className="text-xs text-gray-500">
        A separate, cost-only calculation that&apos;s never combined with the carbon numbers.
      </p>
    ...
    <div className="space-y-6 rounded-xl border bg-white p-6 shadow-sm">
      <div className="grid gap-4 sm:grid-cols-3">
        <Stat
          ...
          value={`${result.currency} ${result.baselineCost.toFixed(2)}`}
        />
        <Stat
          label="Shifted schedule"
          value={`${result.currency} ${result.optimizedCost.toFixed(2)}`}
          highlight
        />
        <Stat
          label="What you'd save"
          value={`${result.currency} ${result.savingsCost.toFixed(2)} (${result.savingsPercent.toFixed(1)} %)`}
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={240}>
          <ComposedChart data={chartData}>
            ...
            <Tooltip
              formatter={(v, name) => {
                if (name === "Day-ahead price") return [`${currency} ${v}/MWh`, name];
                return [`${v} kWh`, name];
              }}
