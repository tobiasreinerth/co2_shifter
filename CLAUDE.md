...
pip install -e ".[dev]"
export DAGSTER_HOME=$(pwd)/.dagster_home  # persists run history + schedule state across restarts
...
- `ShiftCalculator` / `CostShiftCalculator` — each owns its own independent `DataMode` state (default `"avg91"`, i.e. "3 months"), deliberately decoupled from `Co2Chart`/`PriceChart`'s period selects so exploring a shift/magnitude combo against a different window never requires scrolling back to Step 2. Shares only `region` and `loadSlots` with the rest of the dashboard. Both always run the single bounded-reshape approach (`optimizeBoundedReshape()` / `optimizeBoundedCostReshape()` in `lib/cost-shift-calculator.ts`) — there's no separate "rigid whole-day shift" mode, since that's exactly the `magnitudeBand = 0` special case of bounded reshape (only the time-shift search applies). The shift search is jointly exact with the resize step: every candidate shift is scored on its *post-resize* cost (one water-fill per candidate shift, ≤ ~17 for the largest shift-window option), not the pre-resize rigid-shift cost, so a shift that looks worse unshifted but unlocks a cheaper reshape is never missed. Each slot moves ≤ a user-chosen max shift (30/60/120 min) and rescales ≤ a user-chosen magnitude band (0/10/20/30%, where 0% means "shift only, no resizing"), clipped to the profile's own daily max/non-zero-min, total daily energy conserved exactly via a cheapest-slot-first water-filling allocation — the exact optimum for this box-constrained linear-cost problem, not a heuristic. The mechanism is explained as a short bullet list (not prose), including the explicit max/min guarantee and the shift-once-then-resize-per-slot order. Results also report the chosen shift (`summarizeReshape()` in `lib/shift-calculator.ts`) and the single largest per-slot increase/decrease (old time → new time, with a tie count when multiple slots share the extreme). Both render through one shared result view: original schedule (black line), shifted schedule (dark green for CO2 / blue for cost line), and the period's grid CO2 intensity or day-ahead price per slot (grey bars on a compressed secondary axis, deliberately smaller than the load curves) — the two panels' results (buttons "Optimize my carbon savings" / "Optimize my cost savings") are never combined into one number. A stale-result guard (`useEffect` on region/period/`loadSlots`) clears any previously-computed result the moment an input changes, so a number is never shown against inputs it wasn't computed from.
