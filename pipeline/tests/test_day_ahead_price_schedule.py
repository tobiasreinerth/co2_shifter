"""Tests for the daily ENTSO-E day-ahead price ingest schedule."""
...
def test_daily_schedule_requests_every_region_for_last_two_days() -> None:
    ...
    requests = list(daily_day_ahead_price_schedule(ctx))

    assert len(requests) == len(REGION_TO_EIC)
    assert all(isinstance(r, RunRequest) for r in requests)

    configs = {
        r.run_config["ops"]["day_ahead_prices_entsoe"]["config"]["region"]: r for r in requests
    }
    assert set(configs) == set(REGION_TO_EIC)

    de_config = configs["DE"].run_config["ops"]["day_ahead_prices_entsoe"]["config"]
